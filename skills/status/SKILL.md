...
1. **Read the board and the issue list:**
   ...
   - `gh issue list --state open --limit 200 --json number,title,labels,createdAt` — every open issue, to find ones missing from the board.
...
5. **Counts:**
   ...
   - Number of items moved to Done recently (last ~7 days by `updatedAt`, or the most recent handful) — a throughput signal.
